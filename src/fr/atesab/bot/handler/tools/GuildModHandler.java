...
public class GuildModHandler extends ToolHandler {
	...
	public static String getPerm(WebInformation info, IRole r, String title) {
		String s = "<a href='javascript:switchView(\"perm_"+r.getLongID()+"\")'>"+info.getBotServer().getLanguage("tools.guildmod.perm")+"</a> - " + title
		...
		for (Permissions p: pl) {
			String ck = "";
			if(arrayContainObject(p, pl2)) ck = " checked";
			s+="\n<input type='checkbox'"+ck+" name='"+r.getLongID()+"_"+p.name()+"' /> - "+info.getBotServer().getLanguage("tools.guildmod.perm."+p.name())+"<br />";
		}
		return s+"\n<input type='submit' value='"+info.getBotServer().getLanguage("tools.guildmod.perm.save")+"' /></form></div>";
	}
	...
	public String handle(WebToolInformation info) throws IOException {
		...
		if(info.getPost().containsKey("action") && (action=(String) info.getPost().get("action"))!=null){
			if(action.toLowerCase().equals("kick") && BotServer.mapContainKeys(info.getPost(), new String[] {"user"})) {
				try {
					IUser u1 = info.getBotInstance().getClient().getUserByID(Long.valueOf((String)info.getPost().get("user")));
					IVoiceChannel vc = info.getServer().createVoiceChannel(UUID.randomUUID().toString());
					u1.moveToVoiceChannel(vc);
					vc.delete();
					info.setNotification(info.getBotServer().getLanguage("tools.guildmod.channel.users.kick.notif", u1.getName()));
				}catch (Exception e) {}
			} else if(action.toLowerCase().equals("join") && info.getPost().containsKey("channel")) {
				try {
					IVoiceChannel vc = info.getServer().getVoiceChannelByID(Long.valueOf((String)info.getPost().get("channel")));
					vc.join();
				...
				try {
					...
					info.setNotification(info.getBotServer().getLanguage("tools.guildmod.role.duplicate.notif", role.getName(), newRole.getName()));
				}catch (Exception e) {}
		...
		if(info.getClient()!=null) {
			...
			if(g!=null) {
				s+="\n<hr />\n"+UserModHandler.serverElement(g, info.getAccount(), info)+"\n"
						+ "\n<h3 id='role_edit'>"+info.getBotServer().getLanguage("tools.guildmod.role")+"</h3>\n<table class='table_list'>\n"
						+ "<tr class='table_list_top'>"
						+ "<td>"+info.getBotServer().getLanguage("tools.guildmod.role.name")+"</td>"
						+ "<td class='usermod_account'>"+info.getBotServer().getLanguage("tools.guildmod.role.accounts")+"</td>"
						+ "<td class='usermod_id'>"+info.getBotServer().getLanguage("tools.guildmod.role.id")+"</td></tr>";
				...
				for (IRole r: rl) {
					Color c = r.getColor();
					s+="\n<tr id='role_"+r.getLongID()+"' style='border:1px solid rgb("+c.getRed()+", "+c.getGreen()+", "+c.getBlue()+"); background-color:rgba("+c.getRed()+", "+c.getGreen()+", "+c.getBlue()+", 0.25)'>"
							+ "<td>"+"\n<form action='' method='POST' class='inline'>"
								+ "\n<input type='hidden' name='guild' value='"+g.getLongID()+"' />"
								+ "\n<input type='hidden' name='role' value='"+r.getLongID()+"' />"
								+ "\n<input type='hidden' name='action' value='duplicate' />"
								+ "\n<input type='submit' value='"+info.getBotServer().getLanguage("tools.guildmod.role.duplicate")+"' />"
								+ "\n</form>";
						if(!r.isEveryoneRole())
							s+="<form action='' method='POST' class='inline'>"
								...
								+ "\n<input type='submit' value='"+info.getBotServer().getLanguage("tools.guildmod.role.delete")+"' />"
								+ "\n</form> ";
				...
				s+="</table>\n<h3 id='invites'>"+info.getBotServer().getLanguage("tools.guildmod.invites")+"</h3>\n<table class='table_list'>\n"
						+ "<tr class='table_list_top'>"
						+ "<td>"+info.getBotServer().getLanguage("tools.guildmod.invites.link")+"</td>"
						+ "<td class='usermod_account'>"+info.getBotServer().getLanguage("tools.guildmod.invites.uses")+"</td>"
						+ "<td class='usermod_id'>"+info.getBotServer().getLanguage("tools.guildmod.invites.author")+"</td></tr>";
				for (IExtendedInvite i: g.getExtendedInvites()) {
					s+="<tr><td><a href='https://discord.gg/"+i.getCode()+"'>https://discord.gg/"+i.getCode()+"</a></td><td>"+i.getUses();
					if(i.getMaxUses()!=0)s+="/"+i.getMaxUses();
					s+="</td><td>";
					if(i.getInviter()!=null)s+=i.getInviter().getName()+"#"+i.getInviter().getDiscriminator();
					s+="</td></tr>";
				}
				
				s+="</table>\n<h3 id='bans'>"+info.getBotServer().getLanguage("tools.guildmod.bans")+"</h3>\n<table class='table_list'>\n"
						+ "<tr class='table_list_top'>"
						+ "<td>"+info.getBotServer().getLanguage("tools.guildmod.bans.username")+"</td>"
						+ "<td class='usermod_id'>"+info.getBotServer().getLanguage("tools.guildmod.role.id")+"</td></tr>";
				for (IUser u: g.getBannedUsers()) {
					s+="<tr><td>"+u.getName()+"#"+u.getDiscriminator()+"</td><td>"+u.getLongID()+"</td></tr>";
				}
					
				s+="</table>\n<h3 id='text_edit'>"+info.getBotServer().getLanguage("tools.guildmod.channel.text")+"</h3>\n<table class='table_list'>\n"
						+ "<tr class='table_list_top'>"
						+ "<td>"+info.getBotServer().getLanguage("tools.guildmod.channel.name")+"</td>"
						+ "<td class='usermod_id'>"+info.getBotServer().getLanguage("tools.guildmod.channel.id")+"</td></tr>";
				for (IChannel ch: g.getChannels()) {
					s+="<tr><td># "+ch.getName();
					if(ch.isNSFW()) s+="<span class='ch_nsfw'>"+info.getBotServer().getLanguage("tools.guildmod.channel.nsfw")+"</span>";
					s+="</td><td>"+ch.getLongID()+"</td></tr>";
				}
				s+="</table><h3 id='voice_edit'>"+info.getBotServer().getLanguage("tools.guildmod.channel.voice")+"</h3>\n<table class='table_list'>\n"
						+ "<tr class='table_list_top'>"
						+ "<td>"+info.getBotServer().getLanguage("tools.guildmod.channel.name")+"</td>"
						+ "<td class='guildmod_ch_client'>"+info.getBotServer().getLanguage("tools.guildmod.channel.users")+"</td>"
						+ "<td class='usermod_id'>"+info.getBotServer().getLanguage("tools.guildmod.channel.id")+"</td></tr>";
				for (IVoiceChannel ch: g.getVoiceChannels()) {
					s+="<tr><td><form action='' method='POST' class='inline'>"
								...
								+ "<input type='submit' value='"+info.getBotServer().getLanguage("tools.guildmod.channel.join")+"' />"
					...
					for(IUser u: ul) {
						s+="\n<div>\n<form action='' method='POST' class='inline'>"
								...
								+ "<input type='submit' value='"+info.getBotServer().getLanguage("tools.guildmod.channel.users.kick")+"' />"
								+ "</form>\n<img src='"+u.getAvatarURL().replaceAll("webp", "png")+"' style='width:16px;height:16px;' /> ";
						if(info.getAccount().hasPerm("manageclients"))s+="<a href='tools.ap"+info.getCurrentLink()+"&app="+toolName()+"#"+u.getLongID()+"'>";
