...
public class UserModHandler extends ToolHandler {
	private static UserModHandler instance;
	public static UserModHandler getInstance() {
		return instance==null?(instance=new UserModHandler()):instance;
	}
	...
	public static String name(WebInformation info, IUser u, IGuild g, boolean showRealName) {
		...
		if(u.isBot())s+=" <span class='usermod_bot'>"+info.getBotServer().getLanguage("tools.usermod.bot").toUpperCase()+"</span>";
		try{if(g.getClient().getApplicationClientID().equals(String.valueOf(u.getLongID())))s+=" <span class='usermod_me'>"+
		info.getBotServer().getLanguage("tools.usermod.me").toUpperCase()+"</span>";}catch(Exception e) {}
		return s;
	}
	public static String serverElement(IGuild g, Account acc, WebInformation info) {
		String s = "<table class='table_list usermod'><tr class='table_list_top'>"
				+ "<td>"+info.getBotServer().getLanguage("tools.usermod.guild.icon")+"</td>"
				+ "<td>"+info.getBotServer().getLanguage("tools.usermod.guild.name")+"</td>";
			if(acc.hasPerm("guildmod"))s+="<td class='usermod_gtool'>"+info.getBotServer().getLanguage("tools.usermod.guild.tool")+"</td>";
			s+="<td class='usermod_id'>"+info.getBotServer().getLanguage("tools.usermod.guild.id")+"</td></tr>"
				+ "\n<tr>"
				+ "\n<td class='usermod_gimg'><img src='"+g.getIconURL()+"' alt='"+info.getBotServer().getLanguage("tools.usermod.guild.icon")+"' /></td>"
				+ "\n<td>"+g.getName()+"</td>";
			if(acc.hasPerm("guildmod")) s+="\n<td>\n <ul>"
				+ "\n  <a href='tools.ap"+info.getCurrentLink()+"&app="+GuildModHandler.getInstance().toolName()+"#text_edit'><li>"+
					info.getBotServer().getLanguage("tools.guildmod.channel.text")+"</li></a>"
				+ "\n  <a href='tools.ap"+info.getCurrentLink()+"&app="+GuildModHandler.getInstance().toolName()+"#voice_edit'><li>"+
					info.getBotServer().getLanguage("tools.guildmod.channel.voice")+"</li></a>"
				+ "\n  <a href='tools.ap"+info.getCurrentLink()+"&app="+GuildModHandler.getInstance().toolName()+"#role_edit'><li>"+
					info.getBotServer().getLanguage("tools.guildmod.role")+"</li></a>"
				+ "\n  <a href='tools.ap"+info.getCurrentLink()+"&app="+GuildModHandler.getInstance().toolName()+"#invites'><li>"+
					info.getBotServer().getLanguage("tools.guildmod.invites")+"</li></a>"
				+ "\n  <a href='tools.ap"+info.getCurrentLink()+"&app="+GuildModHandler.getInstance().toolName()+"#bans'><li>"+
					info.getBotServer().getLanguage("tools.guildmod.bans")+"</li></a>"
				+ "\n  <a href='tools.ap"+info.getCurrentLink()+"&app="+ UserModHandler.getInstance().toolName()+"#user_edit'><li>"+
					info.getBotServer().getLanguage("tools.usermod")+"</li></a>"
				+ "\n </ul>\n</td>";
			s+="\n<td>"+g.getLongID()+"</td>"
				+ "\n</tr></table>";
		return s;
	}
	private UserModHandler() {
	}
	private String getIconUrl(String url) {
		if(url.toLowerCase().endsWith("null.png")) url = "https://discordapp.com/assets/0e291f67c9274a1abdddeb3fd919cbaa.png";
		return url;
	}
	public String handle(WebToolInformation info) throws IOException {
		String s = "";
		if(info.getClient()==null) return s;
		...
		if(g!=null) {
			...
			if(ul!=null) {
				s+="\n<table id='user_edit' class='table_list usermod'>\n<tr class='table_list_top'>"
						+ "<td>"+info.getBotServer().getLanguage("tools.usermod.avatar")+"</td>"
						+ "<td>"+info.getBotServer().getLanguage("tools.usermod.username")+"</td>"
						+ "<td class='usermod_account'>"+info.getBotServer().getLanguage("tools.usermod.account")+"</td>"
						+ "<td class='usermod_role'>"+info.getBotServer().getLanguage("tools.usermod.role")+"</td>"
						+ "<td class='usermod_id'>"+info.getBotServer().getLanguage("tools.usermod.id")+"</td></tr>";
				...
				for (IUser u: ul) {
					s+="\n<tr id='"+u.getLongID()+"'><td class='usermod_img'><img src='"+getIconUrl(u.getAvatarURL().replaceAll("webp", "png"))+"' alt='"+
							info.getBotServer().getLanguage("tools.usermod.avatar")+"' /></td>"
						+ "<td>"+name(info, u, g)+"</td>";
					...
					for (IRole r: rl) {
						...
						s+="\n<a href='tools.ap"+info.getCurrentLink()+"&app=guildmod#role_"+r.getLongID()+"' "
								+ "style='display:block;padding:2px;margin:1px;border:1px solid rgb("+c.getRed()+", "+c.getGreen()+", "+c.getBlue()+"); "
										+ "background-color:rgba("+c.getRed()+", "+c.getGreen()+", "+c.getBlue()+", 0.25)'>"+r.getName()+"</a>";
					...
					s+="<form method='post' action=''>\n<input type='hidden' value='addrole' name='action' />"
						...
						+ "<br /><input type='submit' value='"+info.getBotServer().getLanguage("tools.usermod.role.add")+"' />\n</form>";
